...
inputEl.addEventListener("input", _.debounce((0, _fetchCountries.fetchCountries), DEBOUNCE_DELAY));

},{"./css/styles.css":"1CY4s","./fetchCountries":"fTQqv","lodash":"3qBDj"}],"1CY4s":[function() {},{}],"fTQqv":[function(require,module,exports) {
...
function fetchCountries({ name =inputEl.value  }) {
    return fetch(API_URL_COUNTRIES + name.trim() + "?fields=name,capital,population,flags,languages").then((response)=>{
        ...
        } else if (countries.length === 0) (0, _notiflixDefault.default).Notify.failure("Oops, there is no country with that name");
        else (0, _notiflixDefault.default).Notify.info("Too many matches found. Please enter a more specific name.");
    }).catch((error)=>{
